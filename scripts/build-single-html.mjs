...
const [template,css,engine,bookEngine,ui]=await Promise.all([
 ...
 readFile(new URL('../src/book-engine.cjs',import.meta.url),'utf8'),
 readFile(new URL('../src/name-ui.js',import.meta.url),'utf8')
]);
const html=template.replace('__INLINE_CSS__',css).replace('__INLINE_ENGINE__',engine).replace('__INLINE_BOOK_ENGINE__',bookEngine).replace('__INLINE_UI__',ui);
